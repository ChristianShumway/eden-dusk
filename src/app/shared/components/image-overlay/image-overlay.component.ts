  ...
  template: `
    @if (shouldShowOverlay()) {
      <div class="fixed inset-0 bg-black bg-opacity-80 z-[51] flex items-center justify-center">
        <div class="relative">
          <img [src]="img" class="max-w-full max-h-screen rounded-lg p-4 md:p-0" />
        </div>
        <button (click)="close()" class="absolute w-4 h-4 top-4 right-4 text-white bg-black rounded-full p-4 cursor-pointer flex justify-center items-center">✕</button>
      </div>
    }
  `
})
